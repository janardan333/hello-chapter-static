...
$(document).on("change", "#hearAboutUs", function () {
  ...
  if (!selectedValue) { 
      $(this).parent().addClass("error").removeClass("update-selected-option");
  } else {
      $(this).parent().removeClass("error").addClass("update-selected-option");
  }
});
...
jQuery(document).ready(function ($) {
  ...
  $(".modal-contact-thank-you .btn-close").click(function (e) {
    ...
    setTimeout(() => {
        window.location.href = "/contact-chapter-home-renovation/";
    }, 100);
});
...
function submitForm(e) {
  ...
  inputs.each((function () {
    ...
    // if (!postDataObject.hearAboutUs || postDataObject.hearAboutUs === "") {
    //   isValid = false;
    //   $("#hearAboutUs").parent().addClass("error");
    // }
  }));
